...
type Mote = { x: number; y: number; z: number; r: number; vx: number; vy: number; phase: number; speed: number };
type Streak = { x: number; y: number; vx: number; vy: number; life: number; max: number; length: number };

const TAU = Math.PI * 2;
const LINK_RADIUS = 170;
const PAIR_RADIUS = 110;

export function startAmbientField(canvas: HTMLCanvasElement): () => void {
